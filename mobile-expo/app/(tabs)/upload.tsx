            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: '#000' }} edges={['top']}>
                ...
                <View style={{ flex: 1 }}>
                    <View style={{
                        ...
                        <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>Capture {isDocMode ? 'Documents' : 'Photos'}</Text>
                        <View style={{ width: 24 }} />
                    </View>
                    ...
                    <View style={{
                        ...
                        backgroundColor: 'rgba(0,0,0,0.6)', paddingBottom: insets.bottom + 20, paddingTop: 20
                    }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 30 }}>
                            ...
                            <TouchableOpacity onPress={isDocMode ? captureScan : capturePhoto} disabled={isProcessing} style={{ alignItems: 'center' }}>
                                ...
                                <Text style={{ color: '#ccc', fontSize: 10, marginTop: 5 }}>{isDocMode ? 'Scan Doc' : 'Photo'}</Text>
                            </TouchableOpacity>
                ...
                {fileQueue.length > 0 && (
                    <TouchableOpacity
                        onPress={() => setMode(selectedProject ? 'folder' : 'project')}
                        style={{
                            position: 'absolute', bottom: insets.bottom + 120, right: 20,
                            backgroundColor: colors.primary, paddingHorizontal: 20, paddingVertical: 12, borderRadius: 30,
                            flexDirection: 'row', alignItems: 'center', gap: 8, elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 3
                        }}
                    >
                        <Text style={{ color: '#fff', fontWeight: 'bold' }}>Next ({fileQueue.length})</Text>
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border }}>
                    ...
                    <Text style={{ fontSize: 15, fontWeight: '700', color: colors.text }}>
                        {mode === 'project' ? 'Select Project' : (selectedFolder ? 'Review & Upload' : 'Select Folder')}
                    </Text>
                </View>
                ...
                <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
                    ...
                    {(mode === 'folder' || mode === 'review') && (
                        <View style={{ gap: 20 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                                        {currentBrowseFolders.map((f) => (
                                            <TouchableOpacity
                                                ...
                                                style={{
                                                    width: '31%', aspectRatio: 1, borderRadius: 12, backgroundColor: colors.surface,
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    if (!selectedFolder) setSelectedFolder(folderBrowseId || 'root');
                                    else handleUpload();
                                }}
                                style={{
                                    marginTop: 12, height: 52, borderRadius: 16, backgroundColor: colors.primary,
                                    alignItems: 'center', justifyContent: 'center', elevation: 2, shadowColor: colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 5
                                }}
                            >
                                <Text style={{ color: '#fff', fontSize: 15, fontWeight: '800' }}>
                                    {!selectedFolder ? `Use ${folderBrowseId ? browseBreadcrumbs[browseBreadcrumbs.length - 1]?.name : 'Root Folder'}` : `Confirm & Upload ${fileQueue.length} Files`}
                                </Text>
                            </TouchableOpacity>
                        </View>
                    )}
                ...
                <Modal visible={showCreateFolder} transparent animationType="fade">
                    <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
                        <View style={{ backgroundColor: colors.surface, borderRadius: 16, padding: 20 }}>
                            <Text style={{ fontSize: 16, fontWeight: '700', color: colors.text, marginBottom: 8 }}>New Folder</Text>
                            <TextInput
                                value={newFolderName} onChangeText={setNewFolderName} placeholder="Folder Name" placeholderTextColor={colors.textMuted} autoFocus
            ...
            <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 40 }}>
                ...
                {mode === 'done' && (
                    <View style={{ flexDirection: 'column', gap: 12 }}>
                        ...
                        <TouchableOpacity onPress={handleClose} style={{ height: 50, borderRadius: 25, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' }}>
