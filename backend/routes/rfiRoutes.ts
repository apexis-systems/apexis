...
router.post('/', upload.array('photos', 5), checkLimit('rfi'), checkLimit('storage'), createRFI);
router.post('/:id/messages', upload.single('file'), checkLimit('storage'), (req, res) => createConversationMessage({ ...req, params: { ...req.params, itemType: 'rfi' } } as any, res));
router.patch('/:id/status', updateRFIStatus);
router.patch('/:id/response', upload.array('photos', 2), checkLimit('storage'), updateRFIResponse);
router.patch('/:id/seen', markRFISeen);
router.patch('/:id', upload.array('photos', 5), checkLimit('storage'), updateRFI);
