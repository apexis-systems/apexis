...
function UploadPageInner() {
    ...
    const folders = selectedProject && uploadType
        ...
        <div className="max-w-2xl p-8 mx-auto">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        {projects.map((project) => (
                            <button
                                key={project.id}
                                onClick={() => { setSelectedProject(project.id); setStep('type'); }}
                                className="flex flex-col items-center gap-2 group"
                            >
                                <div
                                    className="h-14 w-14 rounded-xl flex items-center justify-center shadow-sm border border-border group-hover:border-accent transition-colors"
            ...
            {step === 'type' && (
                <div className="space-y-3">
                    ...
                    <button
                        onClick={() => { setUploadType('documents'); setStep('folder'); }}
                        className="w-full flex items-center gap-3 rounded-xl bg-card border border-border p-4 text-left hover:border-accent transition-colors"
                    >
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">PDFs, DWG files, drawings</p>
                        </div>
                    </button>
                    <button
                        onClick={() => { setUploadType('photos'); setStep('folder'); }}
                        className="w-full flex items-center gap-3 rounded-xl bg-card border border-border p-4 text-left hover:border-accent transition-colors"
                    >
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">Site photos with metadata</p>
                        </div>
                    </button>
                </div>
            )}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {folders.map((folder) => (
                            <button
                                ...
                                className="flex flex-col items-center gap-1 p-4 rounded-xl bg-card border border-border hover:border-accent transition-colors"
                            >
                    ...
                    {folders.length === 0 && (
                        <div className="mt-8 text-center">
                            ...
                            <p className="mt-2 text-xs text-muted-foreground">No folders available</p>
                        </div>
                    )}
                </div>
            )}
                ...
                <div className="space-y-4">
                    <div className="flex flex-col items-center rounded-xl border-2 border-dashed border-border bg-card p-8">
                        ...
                        <p className="text-sm font-medium text-foreground">
                            Click to select {uploadType === 'documents' ? 'documents' : 'photos'}
                        </p>
                    ...
                    <Button onClick={handleUpload} className="w-full h-11 rounded-xl bg-accent text-accent-foreground hover:bg-accent/90 font-semibold">
                        Upload Files
                    </Button>
                </div>
            )}
