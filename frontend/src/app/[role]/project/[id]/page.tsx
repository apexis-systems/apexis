...
export default function ProjectWorkspace({ params }: { params: Promise<{ role: string, id: string }> }) {
    const { id } = use(params);
    const { user } = useAuth();
    ...
    return (
        <div className="flex flex-1 min-h-[calc(100vh-3.5rem)]">
            ...
            <div className="flex-1 p-8 max-w-4xl">
                ...
                {activeTab === 'documents' && (
                    <ProjectDocuments project={project} user={user} />
                )}
                {activeTab === 'photos' && (
                    <ProjectPhotos project={project} user={user} />
                )}
                {activeTab === 'daily' && !isClient && (
                    <ProjectDailyReports project={project} userRole={user.role} />
                )}
                {activeTab === 'weekly' && !isClient && (
