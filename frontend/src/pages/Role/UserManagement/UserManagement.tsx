...
import { UserPlus, Trash2, ToggleLeft, ToggleRight, Loader2 } from 'lucide-react';
...
const UserManagement = () => {
    ...
    const toggleActive = (id: string | number) => {
        ...
        toast.success('User status updated (local only)');
    };

    const changeRole = (id: string | number, role: UserRole) => {
        setUsers(prev => prev.map(u => (u.id === id ? { ...u, role } : u)));
        toast.success('Role updated (local only)');
    };

    const removeUser = (id: string | number) => {
        setUsers(prev => prev.filter(u => u.id !== id));
        toast.success('User removed (local only)');
    };

    const addUser = () => {
        if (!newName.trim() || !newEmail.trim()) { toast.error('Name and email required'); return; }
        const newUser = { id: `u-${Date.now()}`, name: newName.trim(), email: newEmail.trim(), role: newRole, active: true };
                                ...
                                <tr key={u.id} className="border-b border-border last:border-0 hover:bg-secondary/30 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <button onClick={() => toggleActive(u.id)} className="flex items-center gap-1.5">
                                            ...
                                            <span className={cn('text-xs font-medium', u.active !== false ? 'text-accent' : 'text-muted-foreground')}>
