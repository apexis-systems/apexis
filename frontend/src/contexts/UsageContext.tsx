...
interface UsageContextType {
  ...
  checkLimit: (
    type: keyof UsageData["usage"] | "can_export_reports" | "can_export_handover"
  ) => boolean;
}
...
export const UsageProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const checkLimit = (
    ...
    if (type === "contributors") {
      const seatsLimitTotal =
        usage.seats_limit_total ??
    ...
    if (type === "storage_mb") {
      const storageLimit =
        usage.storage_limit_mb ?? limits.storage_limit_mb ?? 100;
