...
interface ProjectWeeklyReportsProps {
  project: Project;
  userRole: UserRole;
}

const ProjectWeeklyReports = ({ project, userRole }: ProjectWeeklyReportsProps) => {
    ...
    <div className="mt-3">
      ...
      {userRole !== 'client' && (
        <Button className="mb-3 w-full h-9 rounded-lg bg-accent text-accent-foreground hover:bg-accent/90 text-xs font-semibold">
          <Upload className="h-3.5 w-3.5 mr-1.5" /> Upload Weekly Report
        </Button>
      )}
